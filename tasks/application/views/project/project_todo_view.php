	...
	<div class="portlet box white">
		<div class="portlet-title">
			<div class="caption">
				...
				<span class="hidden-480">Todo : <?php echo $project->p_title; ?> | <a href="<?php echo base_url(); ?>user/profile/<?php echo $project_creator_name; ?>" target="_blank" ><?php echo $project_creator_fname." ".$project_creator_lname; ?></a></span>
			</div>
...
<?php if($todo=="install"){ ?>
				...
				<div class="span8 responsive">
					...
					if (!empty($todo_data)) {
						foreach ($todo_data as $todo) { 
							switch ($todo->pt_priority) {
								case 'Normal':
									$prio_label = "label-success";
									break;

								case 'Medium':
									$prio_label = "label-warning";
									break;

								case 'High':
									$prio_label = "label-important";
									break;							
						...
						<div class="tile double bg-grey" style="padding-bottom:50px; height:auto !important;"  >
							
								<span class="label <?php echo $prio_label; ?> pull-right"><?php echo $todo->pt_priority; ?></span>

								<p><?php echo stripslashes($todo->pt_comment); ?></p>
								
								<small class="pull-bottom" >Added : <?php echo date('d M Y H:i:s A',$todo->pt_time) ?></strong>

									<?php if ($project_creator_id==$logged) { ?>
										<a href="#deletedialog<?php echo  $todo->pt_id; ?>"   data-toggle="modal" class="btn mini red" ><i class="icon-trash"></i></a>
									
												<!-- LETS LOAD THE DELETE DIALOG -->
											<div id="deletedialog<?php echo $todo->pt_id; ?>" class="modal hide fade bg-grey" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
												<div class="modal-header">
													
														<h3 id="myModalLabel">Delete ToDo</h3>
												</div>
												...
												<div class="modal-footer">
													...
													<a href="<?php echo base_url(); ?>project/delete-todo/<?php echo $todo->pt_id."/".$purl; ?>" class="btn red" >Delete Todo</a>
												</div>
											</div>
					...
					<?php 
						...
						echo "There is currently no ToDo for this project.<hr />";
					}
				...
				<?php if ($project_creator_id==$logged) {
					...
					<div class="control-group">
						<label for="pt_comment" >Description</label>
					  <div class="controls">

						<textarea name="pt_comment" class="m-wrap large" > </textarea>
	
					   </div>
					</div>

					<div class="control-group">
						<label for="pt_priority" >Priority</label>
					  <div class="controls">

							<select name="pt_priority" >
					...
					<div class="control-group">
					  ...
					  <input type="hidden" name="powner_id" value="<?php echo $project_creator_id; ?>" />
					  	...
					  	<input type="submit" name="new_todo" class="btn green" value="Add A ToDo" />
					  </div>
					</div>
					<?php echo form_close(); ?>
...
<?php }else{ ?>
	...
	<div class="span8 responsive">
		<p>Sorry, ToDo App is not INSTALLED for this project.</p>

		<?php if($this->session->userdata('user_id')==$project_creator_id){ ?>

			<br /><a href="<?php echo base_url()."project/install/todo/".$token."/".$purl; ?>" class="btn green">Install ToDo App Now</a>
