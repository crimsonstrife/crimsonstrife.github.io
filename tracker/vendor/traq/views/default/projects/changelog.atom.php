...
foreach ($milestones as $milestone) {
    $entry = array(
        ...
        'link' => "https://" . $_SERVER['HTTP_HOST'] . Request::base($milestone->href()),
    ...
    foreach ($milestone->tickets->exec()->fetch_all() as $ticket) {
        ...
        if ($ticket->type->changelog and $ticket->status->changelog) {
            $data[] = "{$ticket->type->bullet} <a href=\"https://" . $_SERVER['HTTP_HOST'] . Request::base($ticket->href()) . "\">{$ticket->summary}</a>";
        }
    }
